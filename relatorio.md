...
- **Pedro Henrique Nogueira:** O principal aprendizado deste projeto foi a grande diferença entre a teoria das tecnologias e a complexidade de sua aplicação prática em um ambiente distribuído. Percebi que o maior desafio não estava no código dos serviços em si, mas em fazer com que eles se comunicassem corretamente nos diferentes ambientes. A depuração se tornou a habilidade mais valiosa, aprendendo a "seguir" o erro desde a falha no frontend, passando pelo gateway, até os logs de um pod específico no cluster. Ficou claro que a configuração externa, através de variáveis de ambiente, é fundamental; resolver os erros 503 e ECONNREFUSED não exigiu mudanças na lógica de negócio, mas sim em garantir que cada serviço soubesse o endereço correto de suas dependências. Finalmente, a criação do script de automação consolidou a ideia de que a infraestrutura como código não é um luxo, mas uma necessidade para gerenciar a complexidade de sistemas modernos de forma confiável.
